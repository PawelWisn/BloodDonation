...
const useOptionList = () => {
	...
	}, [optionListRes.fetching, optionListRes.data]);
...
function RegisterDonation() {
		...
		<div className="main-page-content">
			...
			<div className="reg-don-container">
				...
				<div id="not-found-div">
					<div>
						<h3>
							...
							>
								Click here and add it to the catalog
							</span>
						</h3>
					</div>
					<div className={classNames("new-loc-div", { hidden: newLocHidden })}>
						...
						<div id="is-new-loc-mobile-div">
							<ManyRadiobuttonsNoDefault
								name={"new-loc-mobile"}
								labels={["Is a mobile point"]}
								values={"0"}
								ids={["new-loc-mobile-radio"]}
							/>
						</div>
					</div>
				</div>
				...
				<div id="remainder-div">
					<ManyRadiobuttons
						name={"reminderButton"}
						labels={["Remind me when I can donate again"]}
						values={"0"}
						ids={["reminderButton"]}
					/>
				</div>
	...
	function GenerateOptions(props: { options: OptionListType }) {
		...
		return (
			<>
				{allLocalizations.map(({ placeName, address, city }) => (
					<option key={placeName + "dropdown"} value={placeName}>
